...
class HorNetIso(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        self.blocks = nn.Sequential(*[Block(dim=dim, drop_path=dp_rates[i], gconv=gconv,
...
@register_model
def hornet_xlarge_gf(pretrained=False,in_22k=False, **kwargs):
    s = 1.0/3.0
    model = HorNet(depths=[2, 3, 18, 2], base_dim=256, block=Block,
    gnconv=[
        partial(gnconv, order=2, s=s),
        partial(gnconv, order=3, s=s),
        partial(gnconv, order=4, s=s, h=14, w=8, gflayer=GlobalLocalFilter),
